...
const getErrorMessage = (err: any): string => {
  ...
  if (typeof err === 'object') {
    ...
    try {
      ...
      return 'Erro objeto não serializável.';
    }
  }
  return String(err);
};
...
const BAIRRO_DB: Record<string, BairroInfo> = {
  ...
  "258": { rs: "Região Oeste", ra: "Ceilândia", bairro: "Área Rural - Brazlândia" }, // Note: Logic from provided text kept as is, though implies Ceilândia
...
// 1. Import Modal (New Feature with File Upload)
const ImportModal = ({ onClose, onSuccess }: { onClose: () => void, onSuccess: () => void }) => {
  ...
  // Function to clean dirty values (e.g. "123" -> 123)
  const cleanValue = (val: string) => {
    if (!val) return null;
    // Replace invisible characters and BOM
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (file.name.endsWith('.csv') || file.name.endsWith('.txt')) {
           ...
           // Excel handling (using array buffer is safer for xlsx)
           const workbook = XLSX.read(data, { type: 'array' });
           const firstSheetName = workbook.SheetNames[0];
           const worksheet = workbook.Sheets[firstSheetName];
           // Convert to CSV string to reuse our existing robust CSV parser logic
           const csvOutput = XLSX.utils.sheet_to_csv(worksheet, { FS: '\t' }); // Use Tab as delimiter for safety
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Scan the first 50 rows to find the one containing 'NU_NOTIFIC'
      let headerRowIndex = -1;
      let delimiter = '\t'; // Default to tab

      for (let i = 0; i < Math.min(rawRows.length, 50); i++) {
        // Normalize row for detection (remove special chars)
        const rowSample = rawRows[i].toUpperCase().replace(/[\uFEFF\u200B]/g, '');
        
        // Check for key column presence (accounting for potential quotes)
        if (rowSample.includes('NU_NOTIFIC')) {
          headerRowIndex = i;
          // Heuristic to detect delimiter based on this row
      ...
      if (headerRowIndex === -1) {
        throw new Error("Não foi possível encontrar a linha de cabeçalho contendo 'NU_NOTIFIC'. Verifique os dados.");
      }
      ...
      if (notificIndex === -1) {
        throw new Error(`A coluna 'NU_NOTIFIC' não foi encontrada na linha de cabeçalho identificada (Linha ${headerRowIndex + 1}).`);
      }

      const patientsToUpsert: any[] = [];
      const duplicateMap = new Map<string, any>(); // To deduplicate by NU_NOTIFIC within the batch
      ...
      for (let i = headerRowIndex + 1; i < rawRows.length; i++) {
        ...
        headers.forEach((header, index) => {
          ...
          if (value) {
             // Handle Dates (SINAN uses DD/MM/YYYY) - Apply to columns starting with DT_
        ...
        // --- Smart Status Inference (SINAN Logic) ---
        ...
        // Only add if we have the Primary Key
        if (patientObj.NU_NOTIFIC) {
           // Use map to keep only the last occurrence if duplicates exist in pasted data
      ...
      if (uniquePatients.length === 0) {
        throw new Error("Nenhum registro válido encontrado para importação. Verifique se a coluna NU_NOTIFIC está preenchida.");
      }

      // 3. Send to Supabase (Batching is safer, but upsert handles array well)
      const { data, error } = await supabase
        .from('tb_casos')
        .upsert(uniquePatients, { onConflict: 'NU_NOTIFIC' });
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50 rounded-t-xl">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">Importação de Dados SINAN</h2>
              <p className="text-xs text-slate-500">Suporta Excel (.xlsx, .xls) ou CSV/Texto</p>
            </div>
          </div>
        ...
        <div className="flex-1 p-6 overflow-hidden flex flex-col space-y-4">
          ...
          {status === 'success' ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
               ...
               <p className="text-slate-600">
                 {stats.imported} casos foram atualizados/inseridos no banco de dados.
               </p>
            </div>
          ) : (
             <div className="flex flex-col h-full space-y-4">
                 ...
                 <div 
                    ...
                    className="border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-brand-primary hover:bg-slate-50 transition group"
                 >
                    <input 
                        type="file" 
                    ...
                    />
                    <div className="bg-blue-50 text-brand-primary p-3 rounded-full mb-3 group-hover:scale-110 transition">
                        <FileSpreadsheet size={24} />
                    </div>
                    <p className="text-sm font-medium text-slate-700">Clique para selecionar arquivo Excel ou CSV</p>
                    <p className="text-xs text-slate-400 mt-1">Arrastar e soltar não suportado, use o clique.</p>
                 </div>

                 <div className="flex items-center justify-between">
                     <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Ou cole os dados abaixo</span>
                     <button 
                        onClick={loadExampleData}
                        className="text-xs text-brand-primary font-medium hover:underline flex items-center"
                    >
                        <FileInput size={12} className="mr-1" /> Carregar Exemplo
                    </button>
                 </div>

                 <textarea 
                    className="flex-1 w-full p-4 border border-slate-300 rounded-lg font-mono text-xs leading-relaxed focus:ring-2 focus:ring-brand-primary outline-none resize-none bg-slate-50 whitespace-pre"
                    placeholder={`Dados brutos aparecerão aqui após upload...\n\nOu cole manualmente:\nNU_NOTIFIC\tDT_NOTIFIC\tNM_PACIENT...\n083220\t11/02/2025\tLANA CRISTINA...`}
                    value={textData}
          ...
          {status === 'error' && (
            <div className="bg-red-50 text-red-700 p-4 rounded-lg flex items-center text-sm border border-red-200">
              ...
              {typeof log === 'string' ? log : 'Erro desconhecido'}
            </div>
          )}
          
          {(status === 'processing' || (status === 'idle' && log && !textData)) && log && (
             <div className="bg-blue-50 text-brand-primary p-4 rounded-lg flex items-center text-sm border border-blue-200">
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50 rounded-b-xl flex justify-between items-center">
           <div className="text-xs text-slate-500 max-w-md">
             *O sistema detecta automaticamente a linha de cabeçalho. Para Excel, a primeira aba será lida.
           </div>
           <div className="flex space-x-3">
             ...
             {status !== 'success' && (
                <button 
                  ...
                  className="flex items-center px-6 py-2 bg-brand-primary text-white rounded-lg hover:bg-[#093452] transition shadow-md disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium"
                >
...
const Dashboard = ({ data }: { data: Paciente[] }) => {
  ...
  const active = data.filter(p => p.STATUS_CASO === 'Em Tratamento').length;
  const abandoned = data.filter(p => p.STATUS_CASO === 'Abandono').length;
  const abandonmentRate = total > 0 ? ((abandoned / total) * 100).toFixed(1) : '0.0';

  // Group by Region
  const byRegion = data.reduce((acc, curr) => {
    // Ensure region is a string
    const region = typeof curr.ID_MN_RESI === 'string' && curr.ID_MN_RESI ? curr.ID_MN_RESI : 'NÃO INFORMADO';
  ...
  const byStatus = data.reduce((acc, curr) => {
    ...
    const status = typeof curr.STATUS_CASO === 'string' && curr.STATUS_CASO ? curr.STATUS_CASO : 'NÃO AVALIADO';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Painel de Indicadores (SINAN)</h2>
      
      {/* KPI Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-slate-500">Total de Notificações</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-bold text-brand-primary mt-1">{active}</p>
            <p className="text-xs text-green-600 flex items-center mt-1">
              <Activity size={12} className="mr-1" /> Monitoramento contínuo
            </p>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-slate-500">Taxa de Abandono</p>
            <p className="text-3xl font-bold text-red-600 mt-1">{abandonmentRate}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <h3 className="text-lg font-semibold text-slate-800 mb-4">Casos por Município/Região</h3>
              ...
              .map(([region, count]) => (
                <div key={region} className="relative">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-slate-500">{count} casos</span>
                  </div>
        ...
        {/* Status Distribution */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <h3 className="text-lg font-semibold text-slate-800 mb-4">Status do Tratamento</h3>
          <div className="flex items-center justify-center py-8">
            <div className="flex gap-4 flex-wrap justify-center">
              {Object.entries(byStatus).map(([status, count], idx) => {
                 const colors: Record<string, string> = {
                    'Em Tratamento': 'bg-blue-500',
                    'Tratamento Concluído': 'bg-green-500',
                    'Abandono': 'bg-red-500',
                    'Óbito': 'bg-slate-800',
                 };
                 return (
                    <div key={status} className="flex flex-col items-center p-4 bg-slate-50 rounded-lg min-w-[120px]">
                        <div className={`w-3 h-3 rounded-full mb-2 ${colors[status] || 'bg-gray-400'}`}></div>
                        <span className="text-2xl font-bold text-slate-800">{count}</span>
                        <span className="text-xs text-slate-500 uppercase tracking-wide text-center">{status}</span>
                    </div>
                 )
              })}
            </div>
...
// 4. Centered Patient Modal (Updated with 11 Logical Tabs)
const PatientModal = ({ 
  patient, 
  onClose, 
  onSave 
  ...
  onClose: () => void, 
  onSave: (p: Paciente) => Promise<void> 
}) => {
  ...
  const TabButton = ({ id, label, icon: Icon }: any) => (
    <button
      ...
      className={`flex items-center whitespace-nowrap space-x-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
        activeTab === id 
          ? 'border-brand-primary text-brand-primary bg-slate-50' 
  ...
  const InputField = ({ label, field, type = "text", width="full", readOnly=false }: { label: string, field: keyof Paciente, type?: string, width?: string, readOnly?: boolean }) => {
    ...
    const safeValue = (rawValue === null || rawValue === undefined) 
      ? '' 
      : (typeof rawValue === 'object' ? '' : rawValue);

    let colSpanClass = "col-span-1 md:col-span-2"; // Default "full" (half screen)
    ...
    return (
      <div className={colSpanClass}>
        <div className="flex items-center mb-1">
            ...
            <TooltipAjuda fieldKey={field as string} />
        </div>
        <input 
          type={type} 
          value={safeValue}
          ...
          readOnly={readOnly}
          className={`w-full p-2 border border-slate-200 rounded text-sm outline-none transition
            ${readOnly ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'bg-slate-50 focus:bg-white focus:ring-2 focus:ring-brand-primary'}
          `}
        />
      </div>
    );
  ...
  const SectionTitle = ({ title }: { title: string }) => (
    <div className="col-span-1 md:col-span-4 mt-6 border-t border-slate-200 pt-4 mb-2">
      <h4 className="font-bold text-slate-700 uppercase text-xs tracking-wider flex items-center">
        <div className="w-1 h-4 bg-brand-primary mr-2 rounded-full"></div>
        {title}
      </h4>
    </div>
  );
      ...
      <div className="relative w-full max-w-6xl h-[90vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-brand-primary text-white shrink-0">
          <div className="flex items-center gap-3">
             ...
             <div>
                <h3 className="text-lg font-bold leading-tight">Prontuário SINAN</h3>
                <div className="flex items-center gap-2 text-xs text-blue-100">
                   <span className="font-mono bg-white/10 px-1.5 rounded text-white">{typeof formData.NU_NOTIFIC === 'string' ? formData.NU_NOTIFIC : ''}</span>
                   <span className="opacity-80">|</span>
                   <span className="uppercase font-medium tracking-wide">{typeof formData.NM_PACIENT === 'string' ? formData.NM_PACIENT : 'PACIENTE SEM NOME'}</span>
                </div>
             </div>
          </div>
        ...
        {/* Strategic Header (Blue Bar) - Always Visible */}
        <div className="bg-blue-50/50 px-6 py-4 border-b border-blue-100 grid grid-cols-1 md:grid-cols-2 gap-6 items-center shrink-0">
            <div>
                <div className="flex items-center mb-1.5">
                    <label className="block text-xs font-bold text-brand-primary uppercase tracking-wider">Status de Gestão</label>
                    <TooltipAjuda fieldKey="STATUS_CASO" />
                </div>
                <div className="relative">
                   <select 
                     value={typeof formData.STATUS_CASO === 'string' ? formData.STATUS_CASO : 'Em Tratamento'}
                     onChange={(e) => handleChange('STATUS_CASO', e.target.value)}
                     className="w-full pl-3 pr-8 py-2 border border-blue-200 rounded-lg bg-white text-sm font-bold text-slate-700 focus:ring-2 focus:ring-brand-primary focus:border-brand-primary outline-none shadow-sm appearance-none"
                   >
                     <option value="Em Tratamento">Em Tratamento</option>
                     <option value="Tratamento Concluído">Tratamento Concluído (Cura)</option>
             ...
             <div>
                <div className="flex items-center mb-1.5">
                   <label className="block text-xs font-bold text-brand-primary uppercase tracking-wider">Observações / Pendências</label>
                </div>
                <input 
                  type="text" 
                  value={typeof formData.OBSERVACOES === 'string' ? formData.OBSERVACOES : ''} 
                  onChange={(e) => handleChange('OBSERVACOES', e.target.value)} 
                  className="w-full px-3 py-2 border border-blue-200 rounded-lg bg-white text-sm text-slate-700 focus:ring-2 focus:ring-brand-primary outline-none shadow-sm placeholder:text-slate-400"
                  placeholder="Resumo rápido do caso..." 
                />
            </div>
        </div>
        ...
        <div className="flex overflow-x-auto border-b border-slate-200 bg-white custom-scrollbar shrink-0">
          <TabButton id="vigilancia" label="Vigilância" icon={Siren} />
          <TabButton id="pessoal" label="Identificação" icon={User} />
          <TabButton id="endereco" label="Endereço" icon={MapPin} />
          <TabButton id="social" label="Social" icon={Tent} />
          <TabButton id="clinico" label="Clínico" icon={ClipboardList} />
          <TabButton id="exames" label="Exames" icon={Microscope} />
          <TabButton id="tratamento" label="Tratamento" icon={Pill} />
          <TabButton id="mudancas" label="Mudanças" icon={ArrowRightLeft} />
          <TabButton id="contatos" label="Contatos" icon={Phone} />
          <TabButton id="encerramento" label="Encerramento" icon={CheckCircle2} />
          <TabButton id="seguimento" label="Seguimento" icon={FolderOpen} />
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-slate-50/50">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 content-start">
                ...
                {activeTab === 'vigilancia' && (
                  <>
                    <SectionTitle title="Dados da Notificação" />
                    <InputField label="Nº Notificação" field="NU_NOTIFIC" readOnly />
                    <InputField label="Tipo Notificação" field="TP_NOT" width="half" />
                    <InputField label="Agravo" field="ID_AGRAVO" width="half" />
                    <InputField label="Data Notificação" field="DT_NOTIFIC" type="date" width="half" />
                    <InputField label="Ano" field="NU_ANO" width="half" />
                    
                    <SectionTitle title="Origem" />
                    <InputField label="UF Notificação" field="SG_UF_NOT" width="half" />
                    <InputField label="Município Notificação" field="ID_MUNICIP" width="full" />
                    <InputField label="Unidade de Saúde" field="ID_UNIDADE" width="full" />
                    <InputField label="Regional" field="ID_REGIONA" width="half" />
                    <InputField label="Data Digitação" field="DT_DIGITA" type="date" width="half" />
                  </>
                )}
                ...
                {activeTab === 'pessoal' && (
                  <>
                    <SectionTitle title="Identificação Civil" />
                    <InputField label="Nome do Paciente" field="NM_PACIENT" width="wide" />
                    <InputField label="Nome da Mãe" field="NM_MAE_PAC" width="wide" />
                    <InputField label="Data Nascimento" field="DT_NASC" type="date" width="half" />
                    <InputField label="Idade" field="NU_IDADE_N" width="half" />
                    <InputField label="Sexo" field="CS_SEXO" width="half" />
                    <InputField label="Raça/Cor" field="CS_RACA" width="half" />
                    
                    <SectionTitle title="Complementares" />
                    <InputField label="Gestante" field="CS_GESTANT" width="half" />
                    <InputField label="Escolaridade" field="CS_ESCOL_N" width="half" />
                    <InputField label="CNS (SUS)" field="ID_CNS_SUS" width="full" />
                  </>
                )}
                ...
                {activeTab === 'endereco' && (
                   <>
                    ...
                    <InputField label="Logradouro" field="NM_LOGRADO" width="wide" />
                    <InputField label="Número" field="NU_NUMERO" width="half" />
                    <InputField label="Complemento" field="NM_COMPLEM" width="half" />
                    <InputField label="Bairro (SINAN)" field="NM_BAIRRO" width="full" />
                    <InputField label="ID Bairro (TB-DF)" field="ID_BAIRRO" width="half" />
                    <InputField label="Município Residência" field="ID_MN_RESI" width="half" />
                    <InputField label="CEP" field="NU_CEP" width="half" />
                    <InputField label="UF Residência" field="SG_UF" width="half" />
                    
                    <SectionTitle title="Contato & Território" />
                    <InputField label="Telefone" field="NU_TELEFON" width="full" />
                    <InputField label="Ponto Referência" field="NM_REFEREN" width="full" />
                    <InputField label="Zona" field="CS_ZONA" width="half" />
                    <InputField label="Distrito Sanitário" field="ID_DISTRIT" width="half" />
                   </>
                )}
                ...
                {activeTab === 'social' && (
                   <>
                    <SectionTitle title="Populações Especiais e Vulnerabilidades" />
                    <div className="col-span-1 md:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <InputField label="Privado de Liberdade?" field="POP_LIBER" width="half" />
                        <InputField label="Situação de Rua?" field="POP_RUA" width="half" />
                        <InputField label="Profissional de Saúde?" field="POP_SAUDE" width="half" />
                        <InputField label="Imigrante?" field="POP_IMIG" width="half" />
                        <InputField label="Beneficiário Gov.?" field="BENEF_GOV" width="half" />
                    </div>
                   </>
                )}
                ...
                {activeTab === 'clinico' && (
                    <>
                    <SectionTitle title="Diagnóstico Inicial" />
                    <InputField label="Data Diagnóstico" field="DT_DIAG" type="date" width="half" />
                    <InputField label="Forma Clínica" field="FORMA" width="half" />
                    <InputField label="Tipo Entrada" field="TP_ENTRADA" width="full" />
                    
                    <SectionTitle title="Extrapulmonar" />
                    <InputField label="Extrapulmonar 1" field="EXTRAPU1_N" width="half" />
                    <InputField label="Extrapulmonar 2" field="EXTRAPU2_N" width="half" />
                    
                    <SectionTitle title="Comorbidades e Agravos" />
                    <div className="col-span-1 md:col-span-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <InputField label="AIDS" field="AGRAVAIDS" width="half" />
                        <InputField label="Alcoolismo" field="AGRAVALCOO" width="half" />
                        <InputField label="Diabetes" field="AGRAVDIABE" width="half" />
                        <InputField label="Doença Mental" field="AGRAVDOENC" width="half" />
                        <InputField label="Drogas Ilícitas" field="AGRAVDROGA" width="half" />
                        <InputField label="Tabagismo" field="AGRAVTABAC" width="half" />
                    </div>
                    </>
                )}
                ...
                {activeTab === 'exames' && (
                    <>
                    <SectionTitle title="Baciloscopias" />
                    <InputField label="Diagnóstico" field="BACILOSC_E" width="half" />
                    <InputField label="1º Mês" field="BACILOSC_1" width="half" />
                    <InputField label="2º Mês" field="BACILOSC_2" width="half" />
                    <InputField label="3º Mês" field="BACILOSC_3" width="half" />
                    <InputField label="4º Mês" field="BACILOSC_4" width="half" />
                    <InputField label="5º Mês" field="BACILOSC_5" width="half" />
                    <InputField label="6º Mês" field="BACILOSC_6" width="half" />
                    
                    <SectionTitle title="Outros Métodos" />
                    <InputField label="Cultura Escarro" field="CULTURA_ES" width="half" />
                    <InputField label="Teste Rápido Molecular" field="TEST_MOLEC" width="half" />
                    <InputField label="Teste de Sensibilidade" field="TEST_SENSI" width="half" />
                    <InputField label="Histopatologia" field="HISTOPATOL" width="half" />
                    
                    <SectionTitle title="Imagem e Triagem" />
                    <InputField label="Raio-X Tórax" field="RAIOX_TORA" width="half" />
                    <InputField label="HIV" field="HIV" width="half" />
                    <InputField label="Terapia Antirretroviral" field="ANT_RETRO" width="half" />
                    <InputField label="Teste Tuberculínico" field="TESTE_TUBE" width="half" />
                    </>
                )}
                ...
                {activeTab === 'tratamento' && (
                    <>
                    <SectionTitle title="Esquema Terapêutico" />
                    <InputField label="Data Início" field="DT_INIC_TR" type="date" width="half" />
                    <InputField label="Situação Tratamento" field="TRATAMENTO" width="half" />
                    <InputField label="TDO Realizado?" field="TRAT_SUPER" width="half" />
                    
                    <SectionTitle title="Medicamentos Prescritos" />
                    <div className="flex gap-3 col-span-1 md:col-span-4 flex-wrap">
                        {['RIFAMPICIN', 'ISONIAZIDA', 'PIRAZINAMI', 'ETAMBUTOL', 'ESTREPTOMI', 'ETIONAMIDA'].map(med => (
                            <label key={med} className={`flex items-center space-x-2 px-3 py-2 rounded-lg border cursor-pointer transition select-none ${formData[med] === '1' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
                                <input 
                                    type="checkbox" 
                                    checked={formData[med] === '1'} 
                                    onChange={(e) => handleChange(med as keyof Paciente, e.target.checked ? '1' : '0')}
                                    className="rounded text-brand-primary focus:ring-brand-primary h-4 w-4"
                                />
                                <span className="text-sm font-medium">{med.charAt(0) + med.slice(1).toLowerCase()}</span>
                            </label>
                        ))}
                    </div>
                    </>
                ...
                {activeTab === 'mudancas' && (
                    ...
                    <InputField label="Data Mudança" field="DT_MUDANCA" type="date" width="half" />
                    <InputField label="UF Destino" field="UF_TRANSF" width="half" />
                    <InputField label="Município Destino" field="MUN_TRANSF" width="full" />
                    <SectionTitle title="Vínculos" />
                    <InputField label="Prontuário Unidade" field="NU_PRONTUA" width="full" />
                    </>
                )}
                ...
                {activeTab === 'contatos' && (
                    <>
                    <SectionTitle title="Investigação de Contatos" />
                    <InputField label="Nº de Contatos Identificados" field="NU_CONTATO" width="half" />
                    <InputField label="Nº de Contatos Examinados" field="NU_COMU_EX" width="half" />
                    </>
                )}
                ...
                {activeTab === 'encerramento' && (
                    <>
                    <SectionTitle title="Desfecho do Caso" />
                    <InputField label="Situação Encerramento" field="SITUA_ENCE" width="full" />
                    <InputField label="Data Encerramento" field="DT_ENCERRA" type="date" width="half" />
                    <InputField label="Status 9º Mês" field="SITUA_9_M" width="half" />
                    <InputField label="Status 12º Mês" field="SITUA_12_M" width="half" />
                    </>
                )}

                {/* 11. Seguimento */}
                {activeTab === 'seguimento' && (
                    <>
                    <SectionTitle title="Atualizações e Reentradas" />
                    <InputField label="UF Atual" field="SG_UF_AT" width="half" />
                    <InputField label="Município Atual" field="ID_MUNIC_A" width="half" />
                    <InputField label="Notificação Atual" field="NU_NOTI_AT" width="full" />
                    <InputField label="Unidade Atual" field="ID_UNID_AT" width="full" />
                    <InputField label="Data Atualização" field="DT_NOTI_AT" type="date" width="half" />
                    </>
                )}
        ...
        <div className="p-4 border-t border-slate-100 bg-white flex justify-between items-center shrink-0">
          ...
          <div className="flex gap-3">
             <button 
               onClick={onClose}
               className="px-5 py-2.5 text-sm font-medium text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition"
             >
               Cancelar
             </button>
             <button 
               onClick={handleSave}
               disabled={saving}
               className="flex items-center px-6 py-2.5 bg-brand-primary text-white rounded-lg hover:bg-[#093452] transition shadow-md hover:shadow-lg text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed transform active:scale-95 duration-150"
             >
...
const PatientsList = ({ 
  ...
  const itemsPerPage = 100; // Updated to 100

  // --- Helper to get Bairro Info ---
  ...
  // --- Pre-process data for filters and display ---
  const enrichedData = useMemo(() => {
    return data.map(p => {
      // 1. Try to get info via ID_BAIRRO
      let bairroInfo = getBairroInfo(p.ID_BAIRRO);
      let rs = bairroInfo.rs;
      let ra = bairroInfo.ra;
      let inferred = false;

      // 2. Fallback: If ID is missing or not found, try to infer from NM_BAIRRO
      ...
      let monthStr = '';
      if (p.DT_NOTIFIC) {
         // Assuming ISO date YYYY-MM-DD from parser
         monthStr = p.DT_NOTIFIC.substring(0, 7); // YYYY-MM
      }
      return {
        ...p,
        regiaoSaude: rs,
        regiaoAdmin: ra,
        notificMonth: monthStr,
        inferredLocation: inferred
      };
    });
  }, [data]);

  // --- Filter Lists (Dynamic) ---
  ...
  const filtered = enrichedData.filter(p => {
    const matchesSearch = 
      (typeof p.NM_PACIENT === 'string' && p.NM_PACIENT.toLowerCase().includes(search.toLowerCase())) ||
      (typeof p.NU_NOTIFIC === 'string' && p.NU_NOTIFIC.includes(search));
  ...
  const getStatusColor = (status?: string) => {
    switch (status) {
      ...
      case 'Em Investigação': return 'bg-purple-100 text-purple-700';
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="relative w-full md:w-96">
          <input 
            type="text" 
            placeholder="Buscar por nome ou notificação..." 
            value={search}
            onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }}
            className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-primary focus:border-transparent outline-none shadow-sm"
          />
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
         <div className="flex items-center mb-3 text-slate-500 text-xs font-semibold uppercase tracking-wider">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
               <label className="block text-xs font-medium text-slate-500 mb-1">Região de Saúde</label>
               <select 
                 className="w-full p-2 text-sm border border-slate-200 rounded-lg bg-slate-50 outline-none focus:ring-2 focus:ring-brand-primary"
                 value={filterRegiaoSaude}
                 onChange={(e) => { setFilterRegiaoSaude(e.target.value); setCurrentPage(1); }}
               >
                 <option value="">Todas</option>
                 {uniqueRegiaoSaude.map(rs => <option key={rs} value={rs}>{rs}</option>)}
               </select>
            </div>

            {/* Admin Region Filter */}
            <div>
               <label className="block text-xs font-medium text-slate-500 mb-1">Região Administrativa</label>
               <select 
                 className="w-full p-2 text-sm border border-slate-200 rounded-lg bg-slate-50 outline-none focus:ring-2 focus:ring-brand-primary"
                 value={filterRegiaoAdmin}
                 onChange={(e) => { setFilterRegiaoAdmin(e.target.value); setCurrentPage(1); }}
               >
                 <option value="">Todas</option>
                 {uniqueRegiaoAdmin.map(ra => <option key={ra} value={ra}>{ra}</option>)}
               </select>
            </div>

             {/* Month Filter */}
             <div>
               <label className="block text-xs font-medium text-slate-500 mb-1">Mês da Notificação</label>
               <select 
                 className="w-full p-2 text-sm border border-slate-200 rounded-lg bg-slate-50 outline-none focus:ring-2 focus:ring-brand-primary"
                 value={filterMonth}
            ...
            {/* Status Filter */}
            <div>
               <label className="block text-xs font-medium text-slate-500 mb-1">Status do Paciente</label>
               <select 
                 className="w-full p-2 text-sm border border-slate-200 rounded-lg bg-slate-50 outline-none focus:ring-2 focus:ring-brand-primary"
                 value={filterStatus}
         ...
         {(filterRegiaoSaude || filterRegiaoAdmin || filterMonth || filterStatus) && (
            <button 
              onClick={() => {
                  setFilterRegiaoSaude('');
                  setFilterRegiaoAdmin('');
                  setFilterMonth('');
                  setFilterStatus('');
                  setCurrentPage(1);
              }}
              className="mt-3 text-xs text-red-500 hover:text-red-700 font-medium flex items-center"
            >
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3">Região de Saúde</th>
                <th className="px-4 py-3">Região Admin.</th>
                <th className="px-4 py-3">Mês Notif.</th>
              ...
              {paginated.length > 0 ? (
                paginated.map((p) => (
                  <tr key={p.NU_NOTIFIC} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-slate-800">
                      {typeof p.NM_PACIENT === 'string' ? p.NM_PACIENT : '-'}
                    </td>
                    <td className="px-4 py-3 text-slate-500">
                      {typeof p.NU_NOTIFIC === 'string' ? p.NU_NOTIFIC : '-'}
                    </td>
                    <td className="px-4 py-3 text-slate-500">
                      <span className={p.inferredLocation ? "text-orange-600 font-medium" : ""}>
                         {p.regiaoSaude}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-500">
                    ...
                    <td className="px-4 py-3 text-slate-500 capitalize">
                       {formatMonth(p.notificMonth)}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${getStatusColor(p.STATUS_CASO)}`}>
                        {p.STATUS_CASO || 'N/I'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button 
                        onClick={() => onEdit(p)}
                        className="text-brand-primary hover:text-[#093452] font-medium text-xs border border-brand-primary/30 hover:bg-blue-50 px-3 py-1.5 rounded transition"
                      >
                        Prontuário
                      </button>
                    </td>
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-slate-400">
                    {data.length === 0 ? "Nenhum dado importado. Verifique o Supabase." : "Nenhum paciente encontrado com os filtros atuais."}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
            <p className="text-xs text-slate-500">
              Mostrando {Math.min((currentPage - 1) * itemsPerPage + 1, filtered.length)} a {Math.min(currentPage * itemsPerPage, filtered.length)} de {filtered.length}
            </p>
  ...
  const fetchData = async () => {
    ...
    try {
      const { data: tbData, error } = await supabase
        .from('tb_casos')
        .select('*')
        .order('DT_NOTIFIC', { ascending: false });
      
      if (error) {
        console.error('Error fetching data:', error);
      } else {
        setData(tbData as Paciente[] || []);
      }
    } catch (err) {
  ...
  const handleUpdatePatient = async (updated: Paciente) => {
    try {
        const { error } = await supabase
            .from('tb_casos')
            .update(updated)
            .eq('NU_NOTIFIC', updated.NU_NOTIFIC);
        
        if (!error) {
            setData(prev => prev.map(p => p.NU_NOTIFIC === updated.NU_NOTIFIC ? updated : p));
        } else {
      ...
      <aside className="w-64 bg-slate-900 text-white hidden md:flex flex-col">
        <div className="p-6 border-b border-slate-800">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight">TB-DF</h1>
              <p className="text-xs text-slate-400">SINAN Integrado</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <NavItem viewName="pacientes" icon={Users} label="Pacientes" />
           ...
           <button
            ...
            className="w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white transition-all duration-200"
          >
            <Upload size={20} />
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="md:hidden bg-slate-900 text-white p-4 flex justify-between items-center shadow-md z-10">
           <div className="flex items-center space-x-2">
              ...
              <span className="font-bold">TB-DF</span>
           </div>
          ...
          {view === 'home' && (
            <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
              <div className="bg-gradient-to-r from-slate-800 to-slate-900 p-8 rounded-2xl shadow-xl text-white relative overflow-hidden">
                ...
                <p className="text-slate-300 relative z-10 max-w-lg">
                  Plataforma centralizada para monitoramento e gestão de casos de Tuberculose (Base SINAN).
                </p>
                <div className="mt-8 flex gap-4 relative z-10">
                   <button 
                    onClick={() => setView('pacientes')}
                    className="bg-brand-primary hover:bg-[#093452] text-white px-6 py-2 rounded-lg font-medium transition shadow-lg shadow-brand-primary/30"
                   >
                     Acessar Pacientes
                   </button>
                   <button 
                    onClick={() => setShowImportModal(true)}
                    className="bg-white/10 hover:bg-white/20 text-white px-6 py-2 rounded-lg font-medium transition backdrop-blur-sm flex items-center"
                   >
                     <Upload size={18} className="mr-2" />
                     Importar Dados
                   </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 <div onClick={() => setView('dashboard')} className="cursor-pointer bg-white p-6 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition group">
                    <div className="flex items-start justify-between">
                       <div className="p-3 bg-blue-50 text-brand-primary rounded-lg group-hover:bg-brand-primary group-hover:text-white transition">
                          <Activity size={24} />
                       </div>
                    </div>
                    <h3 className="mt-4 text-lg font-bold text-slate-800">Monitoramento</h3>
                    <p className="text-sm text-slate-500 mt-1">Visualize indicadores epidemiológicos em tempo real.</p>
                 </div>
                 ...
                 <div onClick={() => setShowImportModal(true)} className="cursor-pointer bg-white p-6 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition group">
                    ...
                    <h3 className="mt-4 text-lg font-bold text-slate-800">Carga de Dados</h3>
                    <p className="text-sm text-slate-500 mt-1">Importação em massa via copiar/colar de planilhas Excel.</p>
                 </div>
                 ...
                 <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 opacity-75">
                    ...
                    <h3 className="mt-4 text-lg font-bold text-slate-800">Encerramento</h3>
                    <p className="text-sm text-slate-500 mt-1">Funcionalidade para validação de encerramento de casos.</p>
                 </div>
              </div>
            </div>
          )}
